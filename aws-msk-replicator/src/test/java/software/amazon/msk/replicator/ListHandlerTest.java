...
public class ListHandlerTest extends AbstractTestBase {
    ...
    public void handleRequest_SimpleSuccess() {
        // Given
        final ListReplicatorsResponse listReplicatorsResponse =
            ListReplicatorsResponse.builder().replicators(getReplicatorSummary(ReplicatorState.RUNNING)).build();
        when(proxyClient.client().listReplicators(any(ListReplicatorsRequest.class)))
            .thenReturn(listReplicatorsResponse);
        ...
        assertThat(response.getResourceModels().get(0).getReplicatorArn()).isEqualTo(REPLICATOR_ARN);
        assertThat(response.getMessage()).isNull();
        assertThat(response.getErrorCode()).isNull();

        verify(proxyClient.client(), times(1)).listReplicators(any(ListReplicatorsRequest.class));
    }
    ...
    public void handleRequest_Exception(Class<KafkaException> kafkaException, HandlerErrorCode cfnError) {
        // Given
        when(proxyClient.client().listReplicators(any(ListReplicatorsRequest.class))).thenThrow(kafkaException);
        ...
        verify(proxyClient.client()).listReplicators(any(ListReplicatorsRequest.class));
    }
}
